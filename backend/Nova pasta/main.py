...
app = FastAPI(title="Sistema de Gestão de Protocolos", version="2.0.1")
...
-    sb = (sort_by or "data_criacao").trim() if isinstance(sort_by, str) else (sort_by or "data_criacao")
-    sb = sb.strip() if isinstance(sb, str) else "data_criacao"
...
def migrar_datas_antigas(usuario: str = Body(...), senha: str = Body(...)):
    ...
    try:
        ...
        raise HTTPException(statuscode=500, detail=f"Erro na migração: {e}")
...
def excluir_categoria(id: str, usuario: str = Query(...)):
    ...
    if res.deleted_count:
        ...
        return {"ok": True}
    raise HTTPException(statuscode=404, detail="Categoria não encontrada.")
