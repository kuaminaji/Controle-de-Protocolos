...
async function carregarNotificacoes() {
  try {
    ...
    const resp = await fetch(`/api/notificacoes?usuario=${encodeURIComponent(sessao.usuario)}`);
    if (resp.ok) {
...
async function marcarNotificacaoComoLida(id) {
  try {
    const resp = await fetch(`/api/notificacao/${id}/ler`, { method: 'PUT' });
    if (resp.ok) {
...
function ensureFavicon(url = '/static/favicon.svg') {
  try {
    ...
    // tenta favicon.svg, cai para logo.png se der erro de carga
...
function mostrarMensagem(mensagem, tipo = "info", tempo = 5000) {
  ...
  box.style.background = tipo === "erro" ? "#f8d7da" : 
                        tipo === "sucesso" ? "#d1edff" : "#fff3cd";
  box.style.color = tipo === "erro" ? "#721c24" : 
                   tipo === "sucesso" ? "#155724" : "#856404";
  box.style.border = tipo === "erro" ? "1px solid #f5c6cb" : 
                    tipo === "sucesso" ? "1px solid #b8daff" : "1px solid #ffeaa7";
...
const CATEGORIA_OPTIONS = ["RGI", "RCPN", "RCPJ", "RTD", "PROTESTO", "NOTAS"];
...
function renderStatusBadge(status) {
  ...
  switch(statusLower) {
    case "pendente":
      cls += " status-pendente";
      icon = `<svg viewBox="0 0 24 24" width="18" height="18"><circle cx="12" cy="12" r="8" fill="#ffe58f"/></svg>`;
      break;
    case "em andamento":
      cls += " status-andamento";
      icon = `<svg viewBox="0 0 24 24" width="18" height="18"><circle cx="12" cy="12" r="8" fill="#a9d6f8"/></svg>`;
      break;
    case "concluído":
      cls += " status-concluido";
      icon = `<svg viewBox="0 0 24 24" width="18" height="18"><circle cx="12" cy="12" r="8" fill="#b2f2a7"/></svg>`;
      break;
    case "exigência":
      cls += " status-exigencia";
      icon = `<svg viewBox="0 0 24 24" width="18" height="18"><circle cx="12" cy="12" r="8" fill="#ffd6d6"/></svg>`;
      break;
    case "excluido":
      cls += " status-excluido";
      icon = `<svg viewBox="0 0 24 24" width="18" height="18"><circle cx="12" cy="12" r="8" fill="#bbb"/></svg>`;
      break;
    default:
      icon = `<svg viewBox="0 0 24 24" width="18" height="18"><circle cx="12" cy="12" r="8" fill="#eee"/></svg>`;
      break;
  }
  ...
  fetch('/api/usuarios')
    ...
    .then(usuarios => {
      ...
      let options = usuarios.map(u =>
        `<option value="${esc(u.usuario)}">${esc(u.usuario)}</option>`
      ).join('');
      ...
      document.getElementById("form-login").onsubmit = async function(e) {
        ...
        try {
          ...
          if (resp.ok) {
            ...
            salvarSessao(res.usuario, res.tipo);
            carregarSistema();
          } else {
...
async function carregarEstatisticas(forcarAtualizacao = false) {
  try {
    ...
    let attentionItems = [];
    try {
      const attResp = await fetch('/api/protocolo/atencao');
      if (attResp.ok) {
...
function renderizarDashboard(data, attentionItems = []) {
  ...
  // Renderizar gráfico com dados atualizados
  setTimeout(() => renderizarGraficoCategorias(por_cat), 120);
}
...
function navegar(pagina) {
  ...
  document.getElementById("btn-baixar-backup-full").onclick = async function() {
    ...
    fetch('/api/backup/full', {method: 'POST'})
      .then(resp => resp.blob())
      .then(blob => {
        esconderLoader();
        const url = window.URL.createObjectURL(blob);
  ...
  if (isAdmin) {
    ...
    if (btnZerar) {
      btnZerar.onclick = async function() {
        ...
        try {
          const resp = await fetch("/api/admin/zerar-app", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  if (pagina === 'incluir') {
    ...
    async function checkNumeroExistsBackend(numero, checkId) {
      try {
        const resp = await fetch(`/api/protocolo?numero=${encodeURIComponent(numero)}&per_page=1`);
        if (!resp.ok) {
    ...
    cpfInput.addEventListener("blur", async () => {
      ...
      if (cpf.length === 11 && isCpfValido(cpf) && !nomeInput.value.trim()) {
        try {
          const resp = await fetch("/api/protocolo/nome_requerente_por_cpf?cpf=" + cpf);
          if (resp.ok) {
            const data = await resp.json();
...
async function salvarFiltroAtual() {
  ...
  try {
    const sessao = getSessao();
    const resp = await fetch('/api/filtros/salvar', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        nome: nome,
...
async function verHistoricoBusca(id, botaoRef) {
  ...
  if (row.style.display === "none") {
    ...
    try {
      const resp = await fetch(`/api/protocolo/${encodeURIComponent(id)}/historico`);
      if (!resp.ok) {
...
async function findOneProtocolo({ numero = '', cpf = '' } = {}) {
  ...
  const params = [];
  if (numero) params.append ? params.push('numero=' + encodeURIComponent(numero)) : null;
  if (cpf) params.append ? params.push('cpf=' + encodeURIComponent(somenteDigitos(cpf))) : null;
  if (params.length) url += '?' + params.join('&');
  params.push ? null : null; // manter estrutura
  params.length ? null : null; // noop
...
function montarFormularioEditar(p) {
  ...
  document.getElementById("form-editar-protocolo").onsubmit = async function(e) {
    ...
    try {
      const resp = await fetch('/api/protocolo/' + p.id, {
        method: 'PUT',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify(dados)
      });
  ...
  try {
    ...
    if (podeExcluir) {
      document.getElementById("btn-excluir-confirma").onclick = async function() {
        ...
        try {
          const resp = await fetch('/api/protocolo/' + p.id + `?usuario=${encodeURIComponent(sessao.usuario)}`, { 
            method: 'DELETE' 
          });
...
async function salvarEditUsuarioInterface(antigoNome) {
  ...
  mostrarLoader("Salvando alterações...");
  try {
    const resp = await fetch(`/api/usuario/${encodeURIComponent(antigoNome)}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        usuario: novoNome,
...
async function excluirUsuario(nome) {
  ...
  try {
    const resp = await fetch(`/api/usuario/${encodeURIComponent(nome)}?logado=${encodeURIComponent(sessao.usuario)}`, {
      method: 'DELETE'
    });
